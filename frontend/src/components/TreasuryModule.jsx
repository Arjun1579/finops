...
// Hardcoded list of popular tickers for the Autocomplete feature
...
export default function TreasuryModule() {
  ...
  return (
    <div className="bg-gray-50 font-sans text-gray-900 pb-12 w-full">
      {/* Premium Dark Header - Now spans full width but content is constrained to a larger max-width */}
      <header className="bg-slate-900 text-white p-8 pb-16 w-full">
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">AI FinOps & Treasury Module</h1>
            <p className="text-slate-400 mt-1">Quantitative Risk Engine & Predictive Analytics</p>
          </div>
      ...
      {/* Main Content Area - Increased max-width to fix empty space */}
      <main className="max-w-[1600px] w-full mx-auto px-4 sm:px-8 -mt-8">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-6 gap-6">
              ...
              <div className="flex flex-wrap gap-4 bg-slate-50 p-4 rounded-xl border border-gray-200 w-full lg:w-auto">
                <form onSubmit={handleSearch} className="flex gap-2 items-end relative" ref={wrapperRef}>
                  <div>
                    ...
                    <input 
                      ...
                      onFocus={() => tickerInput.length > 0 && setShowSuggestions(true)}
                      placeholder="e.g. AAPL, TSLA" 
                      className="block w-48 mt-1 bg-white border border-gray-300 rounded-lg p-2 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none transition relative z-20" 
                    />
                    
                    {/* Autocomplete Dropdown */}
                    {showSuggestions && suggestions.length > 0 && (
                      <ul className="absolute left-0 top-[60px] w-full bg-white border border-gray-200 shadow-xl rounded-lg max-h-48 overflow-y-auto z-50">
                        {suggestions.map((item) => (
                          <li 
                            key={item.symbol} 
                            onClick={() => selectSuggestion(item.symbol)}
                            className="px-3 py-2 hover:bg-blue-50 cursor-pointer flex justify-between items-center border-b border-gray-50 last:border-0"
                          >
                            <span className="font-bold text-slate-800">{item.symbol}</span>
                            <span className="text-xs text-gray-500 truncate ml-2">{item.name}</span>
                          </li>
                        ))}
                      </ul>
                    )}
